/* PLURA v7.0. No framework, analytics, cookies, remote requests or form persistence. */
(() => {
  'use strict';
  ...
  function setMobile(open, restoreFocus = false) {
    ...
    ['#main', '.site-footer', '#back-top'].forEach(selector => { const el = $(selector); if (el) el.inert = open; });
  ...
  document.addEventListener('click', event => {
    if (!event.target.closest('.site-header')) closeGroups();
  });
